...
def _latest_accepted_trend_summary(config: Dict, year: int | str) -> Path:
    ...
    summary_paths = sorted(warc_dir.rglob("cc_collection_summary_*.csv"), reverse=True)
    rejected: List[str] = []
...
def migrate_collection_interim_layout(config: Dict, *, apply: bool = False) -> Path:
    """Move legacy stage-first collection artifacts into the track-first layout."""
    runid = _utc_runid()
    ...
    for source_stage, target_stage in stage_map.items():
        for track in ("corpus", "trend"):
            source_stage_root = root / source_stage / track
            ...
            for source_dir in sorted(source_stage_root.glob("*/batch_*")):
                ...
                target_dir = root / track / year / batch_label / target_stage
                row: Dict[str, object] = {
    ...
    if apply and conflict_count:
        raise FileExistsError(
            "Cannot apply collection interim layout migration because target paths already exist. "
    ...
    if apply:
        ...
        logger.info("Dry-run only; pass --apply to move collection interim artifacts.")
