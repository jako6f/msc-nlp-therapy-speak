\begin{table}[htbp]
\centering
\caption{Annual trend models for ADHD and Autism LSC trajectories by frame.}
...
\bottomrule
\end{tabular}
\parbox{\linewidth}{\footnotesize Note. Cells report annual OLS slopes as $B(SE)$; $\beta$ is the standardised year coefficient. Salience uses Common Crawl source year and has only an Overall target row; semantic measures use document publication year. $^{\dagger}$ marks series whose linear-model residuals show autocorrelation, so the corresponding linear slope should be interpreted cautiously. Significance markers are descriptive and uncorrected: $^{*}p<.05$, $^{**}p<.01$, $^{***}p<.001$.}
\end{table}
